...
chance at every grid cell---whereas on noise-free signals both separate
the classes perfectly. A noise ladder localises the cause: in stationary
Gaussian noise of the measured power spectrum the classes stay separable
at strong dispersion yet collapse to chance on real strain---so
non-Gaussian ``glitches'' set the single-event limit---while near the
observational bound the inter-class difference is itself below the
single-event noise. For the model, training and noise studied, deep
learning gives no per-event advantage over the Gaussian-optimal matched
filter; since that filter is optimal only under the Gaussian likelihood, a
method that models the non-Gaussian noise remains a route to improvement we
do not exclude. We provide this as a validated, openly reproducible
benchmark against which any future claim of a learned single-event
advantage can be measured.
\end{abstract}

\noindent{\it Keywords\/}: gravitational waves, massive graviton, tests of
general relativity, deep learning, matched filtering, LIGO
...
time~\cite{Dax2021}. More recent work has stress-tested these methods on
realistic detector noise: community mock-data challenges benchmark learned
searches against matched filtering on real O3 strain~\cite{Schaefer2023_MLGWSC},
deep residual architectures are trained directly on non-stationary
noise~\cite{Nousi2023_ResNet}, and normalising-flow inference has been made
statistically exact through importance reweighting~\cite{Dax2023_NIS},
together showing that modern architectures can approach matched-filter
sensitivity for detection and parameter estimation. The corresponding
...
out to answer. Whether single-event discrimination is limited by the
discriminator or by the detector noise is itself an empirical question on
real strain; answering it, and identifying \emph{which} feature of the
noise is responsible, is part of the benchmark's value.
...
\emph{Under the assumed stationary-Gaussian-noise likelihood} this is the
strongest possible matched-filter test of the dispersion hypothesis on a
single event: any non-oracle template bank produces a discriminator no
more powerful than $s_{\rm MF}$. In genuinely non-Gaussian data the
statistic is no longer optimal---a distinction that turns out to be
central to interpreting the results (Sec.~\ref{sec:discussion}).
...
\subsection{Robustness to architecture, seed, and training-set size}
\label{sec:results:ablation}

Two further controls test whether the null is specific to the compact
model or to the modest training sets used above. At the resolvable cell
($\lamg=10^{13}\,\km$, $\rho_{\rm opt}=20$) we repeat the strain benchmark
with (i)~a second, established architecture---a 1D residual CNN of the type
standard in gravitational-wave detection~\cite{Gabbard2018,Nousi2023_ResNet},
of comparable size ($\sim 5\times 10^{6}$ parameters)---alongside the
CNN+Transformer; (ii)~three independent random seeds per configuration; and
(iii)~training sets enlarged to $N=1000$ and $N=2000$ per class, five times
the size used in the main sweep (Fig.~\ref{fig:ablation}). Both
architectures reach $\mathrm{AUC}\approx 1$ on noise-free signals,
confirming that each can learn the dispersion signature. On H1 strain both
remain at chance at every training size and seed: the mean validation AUC
is $0.51\pm 0.03$ (CNN+Transformer) and $0.52\pm 0.02$ (ResNet) at
$N=1000$, and $0.50\pm 0.02$ and $0.53\pm 0.01$ at $N=2000$. Neither a
second, established architecture nor a five-fold increase in training data
shifts the single-event strain AUC away from chance---confirming that the
null is a property of the data rather than of model capacity,
initialisation, or training-set size.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{results/fig_ml_ablation.pdf}
  \caption{Architecture, seed, and training-size ablation at
    $\lamg=10^{13}\,\km$, $\rho_{\rm opt}=20$. Best validation AUC versus
    training-set size for the compact CNN+Transformer (blue) and an
    established 1D ResNet baseline (orange) on H1 strain (circles, mean over
    three seeds at $N=1000$ and $2000$ and two seeds at $N=400$; error bars
    $\pm 1\sigma$), with the noise-free capability
    control for each architecture (stars, AUC $\approx 1$). Both
    architectures remain at the chance line (dotted) on strain for all
    training sizes up to $N=2000$ per class, while both separate the classes
    perfectly in the absence of noise.}
  \label{fig:ablation}
\end{figure}

\subsection{A noise ladder isolates the cause of the null}
\label{sec:results:ladder}

The signal-only control shows that the classes are separable in the
absence of noise, but not \emph{which} property of the observational data
destroys that separability. We localise it by interposing a third noise
condition between the two extremes: stationary, colored Gaussian noise
whose one-sided power spectrum equals the measured H1 PSD. On identical
injections we evaluate the oracle matched filter under three
conditions---signal only, Gaussian colored noise, and observational H1
strain---at $\rho_{\rm opt}=50$ across the $\lamg$ grid
(Fig.~\ref{fig:ladder}). The colored-Gaussian generator is normalised so
that a unit-norm template projected onto pure noise has unit variance in
the inner product of Eq.~\eqref{eq:innerproduct} (measured
$\mathrm{Var}=0.99$); its power therefore matches the PSD the matched
filter assumes.

The ladder resolves the null into two regimes. At strong dispersion
($\lamg\le 10^{13}\,\km$) the two waveform families are nearly orthogonal
and their difference carries a large signal-to-noise ratio
[$\rho_{\rm diff}=\rho_{\rm opt}\sqrt{2(1-\mathcal{M})}\approx 66$ at
$\lamg=10^{12}\,\km$]: in Gaussian noise the oracle statistic separates
them perfectly (AUC $=1.00$), yet on real strain it falls to chance
(AUC $=0.50$). The loss of sensitivity there is caused specifically by the
non-Gaussian, non-stationary content of the strain---blip glitches and
short-duration transients---and not by the signal, the SNR, or a Gaussian
noise floor. At weak dispersion ($\lamg\ge 10^{14}\,\km$), near and above
the observational bound, the inter-class difference is intrinsically small
and the Gaussian-noise AUC itself falls---to $0.56$ at $\lamg=10^{14}\,\km$
and to chance ($\approx 0.49$) by $\lamg=10^{15}\,\km$---so there the
single-event difference-SNR, not the glitch content, is the binding limit. The
physically-relevant regime is thus doubly constrained, and the
counter-intuitive observation that even the oracle statistic is at chance
where the templates are nearly orthogonal is explained: the orthogonality
is realised only in the absence of glitches.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{results/fig_noise_ladder.pdf}
  \caption{Noise ladder: oracle matched-filter AUC versus $\lamg$ at
    $\rho_{\rm opt}=50$ under three noise conditions on identical
    injections---signal only (open circles), stationary colored Gaussian
    noise with the measured H1 PSD (squares), and observational H1 strain
    (filled circles); $90\%$ bootstrap intervals. At $\lamg\le 10^{13}\,\km$
    the classes are trivially separable in Gaussian noise yet at chance on
    real strain, isolating non-Gaussian transients as the cause; at
    $\lamg\ge 10^{14}\,\km$ the Gaussian-noise AUC itself falls toward
    chance as the inter-class difference becomes intrinsically too small.}
  \label{fig:ladder}
\end{figure}

To check that the null is not an artefact of one atypical, glitch-rich
interval, we repeat the real-strain benchmark on four independent,
non-overlapping $\sim 4$-minute windows of the O1 H1 reservoir, each with
its own measured PSD (Fig.~\ref{fig:robustness}). The oracle matched-filter
AUC is $0.51\pm 0.02$ at $\lamg=10^{12}\,\km$ and $0.51\pm 0.02$ at
$\lamg=10^{14}\,\km$ across the four windows---chance in every case---so
the single-event null is stable against the choice of noise stretch within
the reservoir. A wider survey across observing runs and detectors is a
natural extension that the released analysis code supports directly.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{results/fig_noise_robustness.pdf}
  \caption{Robustness of the null to the noise reservoir. Real-strain
    oracle matched-filter AUC across four independent time windows of the
    O1 H1 reservoir, at $\lamg=10^{12}\,\km$ (strong dispersion) and
    $\lamg=10^{14}\,\km$ (near the LVK bound), $\rho_{\rm opt}=50$,
    $N=400$ per class; $90\%$ bootstrap intervals. Every window sits at the
    chance line.}
...
at the training-set scales and SNRs studied here. It is important to state
precisely what this does and does not establish. The oracle matched
filter is optimal only under the assumed stationary-Gaussian-noise
likelihood, so its failure on real strain reflects the breakdown of that
assumption rather than a fundamental single-event information bound. The
noise ladder (Fig.~\ref{fig:ladder}) makes the point concrete: the same
statistic that is defeated by glitches separates the classes perfectly in
Gaussian noise of the same power spectrum. A discriminator that explicitly
modelled the non-Gaussian, non-stationary noise---rather than assuming the
Gaussian likelihood the matched filter is built on---could in principle
exceed $s_{\rm MF}$ on real data, and learning such a noise model is one
plausible route by which a per-event advantage might yet emerge; our null
does not exclude it. What the result does establish is narrower but firm:
for the two architectures, training protocol, noise sample and SNRs studied
here---and across three seeds and training sets up to $N=2000$ per class
(Fig.~\ref{fig:ablation})---the learned discriminators match but do not beat
the Gaussian-optimal matched filter, and neither is useful per event at
GWTC-4.0 SNRs. This is consistent with the LIGO--Virgo--KAGRA practice of
obtaining graviton-mass constraints by combining many events.

\paragraph{Limitations.}\label{sec:limitations}
(i) The noise reservoir is restricted to a single $\sim 17$-minute
segment of O1 H1 strain near GW150914. Within that reservoir the null is
stable across independent noise windows (Fig.~\ref{fig:robustness}), but
other observing runs and detectors carry different glitch populations, and
the sensitivity numbers are not guaranteed to generalise across runs
without the wider survey the released code enables.
...
not modelled.
(iv) The training sets are small by GW deep-learning
standards~\cite{GeorgeHuerta2018,Gabbard2018}; the training-set-size study
of Fig.~\ref{fig:control}(b) and the ablation of Fig.~\ref{fig:ablation},
which enlarges $N$ to $2000$ per class (five times the main sweep), both
show the single-event strain AUC already saturated at chance, because the
per-event variance is set by the strain rather than by the training-set
size. We cannot formally exclude modest gains from still larger datasets
or a wider sampling of the noise, but they appear unlikely in this regime.
(v) The learned discriminators are two of many possible choices; we tested
an established 1D ResNet alongside the CNN+Transformer and found the same
null (Fig.~\ref{fig:ablation}), but specialised models trained on
glitch-rich data---transient-aware or attention-based architectures, or
discriminators that explicitly learn the non-Gaussian noise likelihood---may
...
exploits. We do not claim our architectures are optimal.
...
In the physically-relevant regime the single-event sensitivity is
therefore set by non-Gaussian detector noise---blip glitches and
short-duration transients (Figs.~\ref{fig:glitch},~\ref{fig:ladder})---rather
than by the choice between the two discriminators studied: for the two
architectures, training protocol and the SNRs typical of the current
catalogue, the learned representations match but do not beat the
Gaussian-optimal matched filter, and neither offers a shortcut around the
noise floor that obliges LIGO--Virgo--KAGRA to combine many events for a
useful graviton-mass constraint~\cite{TestGR_arXiv2112}. This is a null methodological
