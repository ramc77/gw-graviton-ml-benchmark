...
and the oracle statistic is at chance \emph{even in Gaussian noise}
(AUC $=0.49$ at $\lamg=10^{14}\,\km$): there the single-event
difference-SNR itself, not the glitch content, is the binding limit. The
...
\begin{figure}[t]
  ...
  \caption{Noise ladder: oracle matched-filter AUC versus $\lamg$ at
    ...
    $\lamg\ge 10^{14}\,\km$ even Gaussian noise gives chance, the
    inter-class difference being intrinsically too small.}
  \label{fig:ladder}
\end{figure}
...
for the architecture, training protocol, noise sample and SNRs studied
here, the compact learned discriminator matches but does not beat the
Gaussian-optimal matched filter, and neither is useful per event at
...
specialised models trained on glitch-rich data---transient-aware or
attention-based architectures, or discriminators that explicitly learn the
non-Gaussian noise likelihood---may give different results, and post-hoc
attribution methods~\cite{SHAP2017} could localise which time--frequency
features a successful discriminator exploits. We do not claim our
...
than by the choice between the two discriminators studied: for this
architecture, training protocol and the SNRs typical of the current
catalogue, the learned representation matches but does not beat the
